...
void externalMergeSort(int argc, char *argv[],int chunks_create, int ram_size,struct stat st)
{
	...
	int* sub_array = (int *)malloc(ram_size*sizeof(int));
	int* temp_array = (int*) malloc(ram_size*sizeof(int));
	int *sorted = (int*)malloc(ram_size*sizeof(int));
	fseek(f, 0, SEEK_SET);
	...
	while(more_input)
	{
		...
		MPI_Scatter(arr,ram_size,MPI_INT,sub_array,ram_size,MPI_INT,0,MPI_COMM_WORLD);
	   	merge_sort(sub_array, 0, i - 1);
		MPI_Gather(sub_array, ram_size, MPI_INT, sorted, ram_size, MPI_INT, 0, MPI_COMM_WORLD);
		if(world_rank==0)
		{
		...
		for(j=0;j<i;j++)
		{
			...
			fprintf(chunks[counter],"%d ",sorted[j]);
...
{
	FILE *output_file = fopen("output_mpi.txt","w");
...
int main(int argc , char * argv[])
{
    ...
    //printf("%llu\n",st.st_size);
    //printf("%f\n",file_size);
    double ram_size = 20000*1024;
	ram_size = ram_size*100;
    int chunks_create =  no_chunks_to_be_created(ram_size,file_size);
    printf("%d\n",chunks_create);
    filegenerator(ram_size*chunks_create);
    //externalMergeSort(argc,argv,chunks_create,ram_size, st);
